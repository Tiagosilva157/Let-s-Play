...
const MemberSchema = z.object({
  ...
  monthly_fee_override: z.coerce.number().min(0).optional().or(z.literal("")),
...
export async function addMember(teamId: string, formData: FormData) {
  ...
  if (!player) {
    const { data, error } = await db.from("players").insert({ name: parsed.data.player_name.trim(), phone }).select("id").single();
  ...
  const { error } = await db.from("team_members").insert({
    team_id: teamId,
    player_id: player.id,
    monthly_fee_override: parsed.data.monthly_fee_override === "" ? null : parsed.data.monthly_fee_override,
...
export async function activateSubscription(memberId: string) {
  ...
  const { data: m } = await db
    .from("team_members")
    .select("id, team_id, player_id, monthly_fee_override, due_day, asaas_subscription_id, teams(name, monthly_fee), players(id, name, phone, asaas_customer_id)")
    .eq("id", memberId).single();
  if (!m) return { error: "Mensalista não encontrado." };
  if (m.asaas_subscription_id) return { error: "Assinatura já existe." };

  const player = m.players as unknown as { id: string; name: string; phone: string; asaas_customer_id: string | null };
  ...
  if (value <= 0) return { error: "Defina um valor de mensalidade maior que zero." };

  const { Asaas } = await import("@/lib/asaas");
  try {
    let customerId = player.asaas_customer_id;
    if (!customerId) {
      const customer = await Asaas.createCustomer({ name: player.name, mobilePhone: player.phone, externalReference: player.id });
      customerId = customer.id;
      await db.from("players").update({ asaas_customer_id: customerId }).eq("id", player.id);
    }
    ...
    console.error(e);
    return { error: "Falha ao criar assinatura no Asaas. Verifique a chave de API." };
  }
}
